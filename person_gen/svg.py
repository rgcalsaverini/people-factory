...
def get_groups(node):
    ...
    groups = [c for c in node.childNodes if getattr(c, 'nodeName', None) == 'g']
    if not groups:
        raise ValueError('No groups found on the svg file. Templates should have at least one.')
    return groups


def attributes_to_dict(attributes):
    return {k: getattr(v, 'value', 'None') for k, v in dict(attributes).items()}
...
def path_differences(base_path, comp_path):
    ...
    for idx in range(len(base_path)):
        ...
        if not isinstance(segment_1, type(segment_2)):
            raise ValueError("Segments of index %d do not have the same type on both paths." % idx)
        ...
        seg_differences = {k: getattr(segment_2, k) - getattr(segment_1, k) for k in keys}
...
def apply_transition_to_path(target, transition, amount):
    for seg_idx, changes in transition.items():
        values = {k: getattr(target[seg_idx], k) + amount * changes[k] for k in changes.keys()}
        target[seg_idx] = type(target[seg_idx])(**values)


def split_css(styles):
    return {s.split(':')[0].strip(): s.split(':')[1].strip() for s in styles.split(';') if len(s) > 1}
    ...
    def get_attributes(self, fills=None):
        ...
        for path_attrs in self.path_attributes.get('default').values():
            ...
            if fills_in_class:
                ...
                new_attr['fill'] = path_attrs.get('fill', None) or styles.get('fill', '#FF0000')
            new_attr['opacity'] = path_attrs.get('opacity', None) or styles.get('opacity', '1')
    ...
    def _parse_layers(self, groups):
        for group in groups:
            ...
            if not attributes.get('inkscape:groupmode', None) == 'layer':
                raise ValueError('Top-level groups should be layers on templates.')
    ...
    def _create_transition_map(self):
        ...
        for layer_id, layer_paths in self.layers.items():
            ...
            for path_id in set(layer_paths.keys()).intersection(default_paths):
                diff = path_differences(self.default[path_id], layer_paths[path_id])
                if diff:
        ...
        for transition_id, transition_elements in self.transition_map.items():
            if transition_id in self.must_provide:
                if transition_id not in descriptors.keys():
                    raise KeyError('Descriptor \'%s\' should be provided' % transition_id)
                amount = descriptors[transition_id]
            else:
                amount = random.uniform(0, 1)
            for element_id, element_paths in transition_elements.items():
                apply_transition_to_path(self.default[element_id], element_paths, amount)
